...
function applyTranscriptEvent(view, event) {
	...
	switch (event.type) {
		...
		case "assistant/attempt": {
			...
			let acc = base.streaming !== void 0 && base.streaming.turn === turn && base.streaming.step === step ? { ...base.streaming } : {
				turn,
				step,
				text: "",
				reasoning: ""
			};
			for (const { chunk } of expandAssistantStream(event.data.stream)) if (chunk.type === "text-delta") acc = {
				...acc,
				text: acc.text + chunk.text
			};
			else if (chunk.type === "reasoning-delta") acc = {
				...acc,
				reasoning: acc.reasoning + chunk.text
			};
			return {
				...base,
				streaming: acc
	...
	async ask(question) {
		...
		const feed = this.ctx.on("session/event", (owner, event) => {
			...
			if (event.type === "assistant/attempt") {
				for (const { chunk } of expandAssistantStream(event.data.stream)) if (chunk.type === "text-delta") buffer.push(chunk.text);
			} else if (event.type === "assistant/message") {
				const message = event.data.message;
				for (const block of message?.content ?? []) if (block.type === "text" && block.text !== void 0) messageText.push(block.text);
...
var TuiApp = class {
	...
	*/
	handleStreamEvent(event) {
		...
		switch (event.type) {
			case "assistant/attempt":
				for (const { time: chunkTime, chunk } of expandAssistantStream(event.data.stream)) if (chunk.type === "text-delta") {
