...
const WELCOME_STYLES = ["star", "retro"];
...
function titleFromEvents(events) {
	...
	return fallbackSessionTitle(first.text, 5, 40);
}
/**
* 计算一个会话在 `/session list` 中的展示标题。
...
function persistenceFacet(ctx) {
	return ctx.reflect !== void 0 ? ctx.reflect.get("sessionPersistence", false) : ctx.get("sessionPersistence");
}
...
function messageText(message) {
	...
	for (const block of message.content) switch (block.type) {
		case "text":
			text += block.text;
			break;
		case "reasoning": reasoning += block.text;
	}
	return {
		text,
		reasoning
...
function formatSessionListLines(rows, now) {
	...
	for (const bucket of groupSessionsByAge(rows, now)) {
		...
		for (const row of bucket.items) out.push(`${row.id} · ${row.title} · ${new Date(row.createdAt).toISOString()}`);
	}
	return out;
}
//#endregion
...
const PALETTE_HEX = Object.fromEntries(STAR_WHALE_FRAME_PALETTE.flatMap((hex, i) => i === 0 || hex === null ? [] : [[i.toString(16), hex]]));
/** level 1 轨：同索引取现场最近邻 ANSI16 命名色（调色板变更免维护近似表）。 */
const PALETTE_ANSI16 = Object.fromEntries(STAR_WHALE_FRAME_PALETTE.flatMap((hex, i) => {
...
const STAR_TITLE_MAX_COLS = STAR_TITLE_ART.standard.width;
const STAR_TITLE_MIN_COLS = STAR_TITLE_ART.mini.width;
/** star hero 宽屏门禁最小列数（gutter + 画 + 间隙 + 标题窄档）。 */
const STAR_HERO_MIN_COLS = 49 + STAR_TITLE_MIN_COLS;
/**
...
function formatStarWelcomeHero(input, theme) {
	const { width, whale, tips } = input;
	if (width <= 0) return [];
	if (width < STAR_HERO_MIN_COLS || input.rows < 24) return [];
...
var TuiApp = class {
	...
	*/
	async attach(initialSessionId) {
		...
		if (typeof userQuestions?.registerProvider === "function") this.interactionDisposer = userQuestions.registerProvider({ ask: (request) => this.handleQuestionRequest(request) });
		this.attached = true;
